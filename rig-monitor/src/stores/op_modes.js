...
export const useOpModeStore = defineStore('op_modes', () => {
    ...
    const get_op_mode = (id) => {
        const op_mode = op_modes.value[id];
        if (!op_mode) {
    ...
    const add_op_mode = (op_mode) => {
        op_modes.value[op_mode.id] = op_mode.asObject();
    ...
    const remove_op_mode = (id) => {
        ...
        if (id === current_op_mode.value) {
            current_op_mode.value = Object.keys(op_modes.value).find(id => id !== id) || '';
            console.warn(`Current op mode "${id}" removed, switching to "${current_op_mode.value}".`);
        }
        delete op_modes.value[id];
    }
    ...
    const get_current_op_mode = computed(() => {
        if (current_op_mode.value === -1 || !op_modes.value.hasOwnProperty(current_op_mode.value)) {
            current_op_mode.value = Object.keys(op_modes.value)[0] || -1;
        }
        return op_modes.value[current_op_mode.value] || null;
    });
    return {
        op_modes,
        get_op_mode,
